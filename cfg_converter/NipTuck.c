...
int main(int argc, char **argv){
    char 	cfgfile[20],output[20],command[50],temp[5],line[80];
    FILE 	*fp,*fo;
    int 	i,j,k=0,frame,Natom,ID_layer_1[MAX],n1=0,ID_layer_2[MAX],n2=0;
	int		center1=294,center2=595;
    double 	H[4][4],s1,s2,s3,radius=1000,n[3]={-1.0887,13472,0};
    struct 	atom {
			int id;
    	    double x,y,z;
    };
    ...
    for(i=0;i<50;i++) command[i]='\0';
    sprintf(command,"grep \"63.546 Cu\" ");
    strcat (command,cfgfile);
...
//  fscanf(fp,"%s",&temp);
    ...
    fclose(fp);
    sprintf(output,"%s.in",cfgfile);
    fo = fopen(output,"w");
  
    fprintf(fo, "Yin Zhang's Input\n");
    fprintf(fo, "\n");
    ...
    for(i=0;i<Natom;i++) {
		...
		for(j=0;j<n2;j++)
			...
			if(frame !=0) continue;
                         	fprintf(fo,"      %d   %d     %g %g %g   0  0  0\n",n1+n2+i+1-k,1,(Cu+i)->x,(Cu+i)->y,(Cu+i)->z);
                         }
    fclose(fo);
